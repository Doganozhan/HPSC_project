...
// peut poser problème avec les structures de branching, le fait de mettre un do...while(0) fait que la macro est vue comme une seule et unique instruction, c'est important car il y a un if à l'intérieur de la macro.
// Si par exemple dans le code on a un if(...){macro}  else{...}, si on met pas le do...while(0), le else du code va être attaché au if de la macro, et pas à celui du code !  
...
__global__ void kernel_update_hx(float *hx, const float *ez, int nx, int ny, float chy)
{
  ...
  if(global_i >= nx || global_j >= ny-1) //Pour quand on alloue "trop de threads", car on les alloue par block. Les deux conditions sont avec un "=", regarder l'instruction suivante pour comprendre: "out of
    return;                            //range access", pour global_j == ny.


  /* Remplissage du tableau shared pour les threads à l'intérieur du domaine (de shared_j == 0 jusque shared_j == 15) */
  shared_ez[shared_j * blockDim.x + shared_i] = ez[global_j * nx + global_i];  // Car le tableau shared_ez possède blockDim.x colonnes et ez, lui en possède nx. Cette instruction ne remplit que les 16 première lignes
  ...
  if(threadIdx.y == (blockDim.y - 1)  ||  global_j == ny-2) // "Si je suis un thread du dessus du domaine...". La deuxième condition n'est utile que pour les derniers blocs, ceux tout au dessus
    shared_ez[(shared_j + 1) * blockDim.x + shared_i] = ez[(global_j + 1) * nx + global_i];

    __syncthreads(); // Avant de lire shared_ez, il faut s'assurer que chaque thread aie fini d'écrire dedans avant de l'utiliser 


  // hx[i,j] = hx[i,j] - chy * (ez[i, j+1] - ez[i, j]);
  float hx_ij = hx[global_j * nx + global_i]
              - chy * (
                shared_ez[(shared_j + 1) * blockDim.x + shared_i] 
              - shared_ez[shared_j * blockDim.x + shared_i]
              );
...
// update hy: i = 0, ... , nx-2 et j = 0, ... , ny-1
__global__ void kernel_update_hy(float *hy, const float *ez, int nx, int ny, float chx)
{

  extern __shared__ float shared_ez[]; // Sa taille sera de (blockDim.x + 1) * blockDim.y
  ...
  // Domaine Hy: i=0, ..., nx-2 et j=0, ..., ny-1
  if(g_i >= nx-1 || g_j >= ny)
    return;

  shared_ez[s_j * (blockDim.x + 1) + s_i] = ez[g_j * nx + g_i]; // Pour les threads au milieu du domaine

  
  if(threadIdx.x == (blockDim.x - 1)  ||  g_i == nx-2)  // "Si je suis un thread à droite du domaine..."
    shared_ez[s_j * (blockDim.x + 1) + (s_i + 1)] = ez[g_j * nx + (g_i + 1)];

  __syncthreads();

                  
  // hy[i,j] = hy[i,j] + chx * (ez[i+1, j] - ez[i, j]);
  float hy_ij = hy[g_j * (nx-1) + g_i] 
              + chx * (
                shared_ez[s_j * (blockDim.x + 1) + (s_i + 1)]
              - shared_ez[s_j * (blockDim.x + 1) + s_i]
              );

  //set(hy, nx, i, j, hy_ij);
  hy[g_j * (nx-1) + g_i] = hy_ij;
}
...
__global__ void kernel_update_ez(float *ez, const float *hx, const float *hy, int nx, int ny, float cex, float cey)
{
  ...
  __shared__ float shared_hy[(BLOCK_X + 1) * BLOCK_Y]; // On n'a pas besoin de ghost column à droite car i = i + 1/2 numériquement !
...
__global__ void kernel_impose_source(float *ez, int nx, int ny, float value)
{
  ...
  if(threadIdx.x == 0 && blockIdx.x == 0  &&  
     threadIdx.y == 0 && blockIdx.y == 0) //Au cas où on lance le kernel avec + de un seul thread (sur grid à 2D)
  {
    const int g_i = nx/2;
    const int g_j = ny/2;
    ez[g_j* nx + g_i] = value;
...
dim3 grid_hx((nx + block.x - 1) / block.x, ((ny-1) + block.y - 1) / block.y);
dim3 grid_hy(((nx-1) + block.x - 1) / block.x, (ny + block.y - 1) / block.y); // Les grids n'ont pas les mêmes dimensions. C'est + opti si par exemple nx = k*blockDim.x + 1, pour (nx-1) on alloue donc un bloc entier en moins
dim3 grid_ez((nx + block.x - 1) / block.x, (ny + block.y - 1) / block.y);

//Tailles des tableaux shared pour les kernels
size_t size_in_bytes_shared_ez_1 = size_t(BLOCK_X) * size_t(BLOCK_Y + 1) * sizeof(float);
size_t size_in_bytes_shared_ez_2 = size_t(BLOCK_X + 1) * size_t(BLOCK_Y) * sizeof(float);
...
for(int n = 0; n < nt; n++)
{
  ...
  kernel_update_hx<<<grid_hx, block, size_in_bytes_shared_ez_1>>>(d_hx, d_ez, nx, ny, chy);
  kernel_update_hy<<<grid_hy, block, size_in_bytes_shared_ez_2>>>(d_hy, d_ez, nx, ny, chx);
  kernel_update_ez<<<grid_ez, block>>>(d_ez, d_hx, d_hy, nx, ny, cex, cey);
