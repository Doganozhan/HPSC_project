...
dim3 grid_hy(((nx-1) + block.x - 1) / block.x, (ny + block.y - 1) / block.y); // Les grids n'ont pas les mêmes dimensions. C'est + opti si par exmple nx = k*blockDim.x + 1, pour (nx-1) on alloue donc un bloc entier en moins
