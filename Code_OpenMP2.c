              ...
              double dy, double val)
{
  ...
  // ATTENTION for(int i = 0; i < nx * ny; i++) data->values[i] = val;  NON !!! Si non c'est que le master NUMA node qui touche les data !!! ATTENTION
  return 0;
}
...
int main(int argc, char **argv)
{
  ...
  int problem_id = 2;  // ATTENTION: j'ai modifié cette ligne
  switch(problem_id) {
  case 1: // small size problem
    dx = dy = (3.e8 / 2.4e9) / 20.; // wavelength / 20
    nx = ny = 500;
    dt = 0.5 / (3.e8 * sqrt(1. / (dx * dx) + 1. / (dy * dy))); // cfl / 2  (Courant-Friedrichs-Lewy/2)
    nt = 500;
  ...
  double start, end; // Pour mesurer le temps d'exécution
  ...
  double cey = dt / (dy * eps);
    ...
    /*************************** Pour tenir compte de la first-touch policy ***************************/
    #pragma omp parallel
    {
        #pragma omp for
        for(int j=0; j<ny; j++)
    ...
    ///Quand on a fait malloc dans init_data juste au dessus, on a alloué de la mémoire virtuelle, on sait qu'on a alloué un tel nombre de bytes mais on ne sait pas encore physiquement où ils seront. C'est pour
    //ça que ici on fait accéder chaque thread aux éléments du tableau auxquels il aura besoin pour calculer les prochaines boucles. La mémoire est physiquement alloué au premier accès (ça aide mais ce n'est pas
    //parfait, il y aura toujours des remote NUMA accesses, au niveau des bords de domaine)
  
  //NOTE: Rien ne sert de mettre private(dx,dy,nx...) car, certes ces variables sont shared par défaut comme elles sont déclarées en dehors de la directive #pragma omp parallel mais on n'écrit pas dedans dans la
  //région parallel => aucune data race possible




  
  #pragma omp parallel  //Il n'y a que les boucles qu'il faut paraléliser, dont on met la directive parallel ici et les for après
  {
    //On commence à enregistrer le temps
    #pragma omp barrier // Pour qu'on enregistre le départ de tous les threads au même moment (pas de barrière implicite avec omp master)
    #pragma omp master
    start = GET_TIME();
    #pragma omp barrier //Idem (Pour que les autres threads attende le master thread qui a fait GET_TIME() juste avant)
    ...
    for(int n = 0; n < nt; n++) {  //Boucle sur tous les pas de temps et affiche régulièrement une estimation du temps restant (ETA)  
      ...
      /*************************** Calcul de la propagation de l'onde dans le réseau ***************************/
      ...
      // update ez
      #pragma omp for //Attention: ici on ne met pas de clause nowait car on a besoin d'avoir calculé tous les Hx(i,j) et Hy(i,j) pour calculer les Ez(i,j)
      for(int j = 1; j < ny - 1; j++) {
        for(int i = 1; i < nx - 1; i++) {
          double ez_ij = GET(&ez, i, j) +
                        cex * (GET(&hy, i, j) - GET(&hy, i - 1, j)) -  //Aucune data race pour accéder à Hx et Hy car ils sont écrits avant ces deux loops pour Ez (penser à la barriere implicite à la fin de chaque directive for)
    ...
    #pragma omp barrier
    #pragma omp master
    end = GET_TIME();  // J'aurais aussi pu utiliser #pragma omp for private(time) reduction(max:time) pour calculer le temps d'exécution...
