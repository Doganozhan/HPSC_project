...
// Création du fichier pour stocker les valeurs des points (dt,|Ez_max|)
  FILE *fichier = fopen("E_max_vs_dx.txt", "w");
  ...
  for(double i = 0 ; i<12 ; i++)
  {
    ...
    case 1: // small size problem
      dx = dy = (3.e8 / x) / 20; // wavelength / 20 (initial frequency of 1e9 Hz) 
      printf("%g\n", dx);
      nx = ny = 500;
      dt = 1.0 / (3.e8 * sqrt(1. / (dx * dx) + 1. / (dy * dy))); // cfl / 2  (Courant-Friedrichs-Lewy/2)
      printf("%g\n", dt);
      nt = 500;
    ...
    //On ecrit les points (dx, |Ez_max|) dans le fichier
    fprintf(fichier, "%g\t%g\n", dx, ez_max);
