...
int main(int argc, char **argv)
{
  ...
  /* check divisibility */
if((nx % Px) != 0 || (ny % Py) != 0) {
  if(rank == 0) fprintf(stderr, "Error: nx,ny must be divisible by Px,Py (nx=%d Px=%d, ny=%d Py=%d)\n", nx, Px, ny, Py);
...
if(rank == 0) {
  ...
  // Subdomain sizes
  const int nx_loc = nx / Px;
  const int ny_loc = ny / Py;
  //Subdomain starts and ends
  int istart = px * nx_loc; 
  int jstart = py * ny_loc;
  ...
  const int left_rank = (rank % Px == 0)      ?  MPI_PROC_NULL : rank - 1;
  ...
  for(int n = 0; n < nt; n++) //Boucle sur tous les pas de temps et affiche régulièrement une estimation du temps restant (ETA)
  {
    ...
    MPI_Sendrecv(send_hx_top, nx_loc, MPI_DOUBLE, up_rank, 10,
  ...
  if(rank == 0) {
    ...
    printf("Performance: %g MUpdates/s\n",
           1e-6 * (double)nx * (double)ny * (double)nt / max_time); //Affiche le temps total d’exécution et la performance en millions de mises à jour par seconde.
  }
